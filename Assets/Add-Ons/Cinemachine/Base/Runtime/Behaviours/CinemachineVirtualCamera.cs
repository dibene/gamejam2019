...
namespace Cinemachine
{
    ...
    public class CinemachineVirtualCamera : CinemachineVirtualCameraBase
    {
		...
		private float dutchFinal;
		private float angYFinal;
		public float difY;
		public float rotY;
        ...
        override public void UpdateCameraState(Vector3 worldUp, float deltaTime)
        {
			...
			//agrego:
			//si Penny mira a la derecha, inclina cámara (dutch) a la derecha. Idem izquierda.
			dutchFinal = 0;//m_Follow.GetComponent<SpriteRenderer> ().flipX ? -1f : 1f; ////////////////////////////////////////////////
			angYFinal  = m_Follow.GetComponent<SpriteRenderer> ().flipX ? -5f : 5f; ////////////////////////////////////////////////

			//lo siguiente es para que se incline de a poco
			float dif = m_Lens.Dutch - dutchFinal;
			if (Math.Abs( dif) > 0.1) {
				m_Lens.Dutch -= dif / 10f;
				//if (dif<0) m_Lens.Dutch += dif / 10f;
			}

			//localRotY = camParent.localRotation.y;
			//rotY = camParent.rotation.eulerAngles.y;
			//difY = angYFinal-rotY;
			//camParent.Ro RotateTowards(transform.rotation, Quaternion.Euler(0, 180, 0), speed * Time.deltaTime);
			float speed = 1F; 
			camParent.rotation = Quaternion.Slerp(transform.rotation, Quaternion.Euler(0, angYFinal, 0), Time.deltaTime * speed);
			/*
			if (Math.Abs( difY) > 0.1) {
				camParent.Rotate (
					0,
					eulerAngles( -(difY/10) ),
					0
					//camParent.rotation.w
				);
			}
			*/
		}
